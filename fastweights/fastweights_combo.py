"""Step 4, decisive experiment: are gradient-free FAST WEIGHTS complementary
to gradient-free MEMORY, or redundant with it?

One live pass over a stream computes, at every position (prequential: score
before update):
  * the frozen LM distribution                       -> base
  * three delta-rule readout adapters (fast weights) -> FW variants
        l' = l_base + A phi(h),  A <- A + step (onehot - p') outer phi(h)
        step = eta * g  (surprise-gated)  or  eta * mean_g  (uniform control,
        matched plasticity budget -- the paper-1 ablation design)
        eta grid extended DOWN to 0.03 (the previous run selected the grid
        edge eta=0.1; eta >= 0.3 diverges)
  * the Sillage n-gram Hebbian tier statistics          -> memory

Post hoc, with dev = first 20%: FW alone, memory alone, and memory routed on
top of the FW-adapted base (same flat-mixture rule as the papers). Paired
block bootstraps answer the question.

Usage: python fastweights_combo.py <domain>
Output: results/fwcombo_<domain>.json
"""
...
R_FEAT, D_G, D_V, NGRAM = 256, 4096, 256, 4
WINDOW, STRIDE = 1024, 512
FW = [("fw_r256_uniform", 0.1, "uniform", 256),
      ("fw_r64_uniform", 0.1, "uniform", 64),
      ("fw_r16_uniform", 0.1, "uniform", 16)]


@torch.no_grad()
def run_pass(domain, vocab=50257):
    ...
    Rf = {r: rng.normal(size=(hidden, r)).astype(np.float32) / np.sqrt(hidden)
          for r in sorted({c[3] for c in FW})}
    ...
    A = {c[0]: np.zeros((vocab, c[3]), dtype=np.float32) for c in FW}
    M_G = np.zeros((D_G, D_V), dtype=np.float32)
    lp_base = np.zeros(n, dtype=np.float32)
    lp_fw = {c[0]: np.zeros(n, dtype=np.float32) for c in FW}
    ...
    while a < n:
        ...
        for i in range(lo, w):
            ...
            g_mean = g_sum / g_cnt
            phis = {}
            for r, Rm in Rf.items():
                v = hs[i] @ Rm
                phis[r] = v / (np.linalg.norm(v) + 1e-8)
            for name, eta, mode, r in FW:
                phi = phis[r]
                la = lb + A[name] @ phi
                m = la.max()
                p = np.exp(la - m)
                p /= p.sum()
                lp_fw[name][j] = np.log(max(p[truth], 1e-30))
                step = eta * (g if mode == "gated" else g_mean)
...
def tune_memory(p_basis, s_true, smax, lse, dev):
    """Route the Hebbian tier on top of an arbitrary base distribution."""
    best = None
...
def main():
    domain = sys.argv[1]
    lp_base, lp_fw, s_true, smax, lse = run_pass(domain)
    n = len(lp_base)
    dev = np.arange(n) < int(0.2 * n)
    test = ~dev
    base_t = float(-lp_base[test].mean())
    p_base = np.exp(lp_base)
    out = {"domain": domain, "n": int(n), "base_nll_test": base_t,
           "memory_bytes": D_G * D_V * 4}

    best_fw, best_dev = None, None
    for name, _e, _m, r in FW:
        dv = float(-lp_fw[name][dev].mean())
        tv = float(-lp_fw[name][test].mean())
        out[name] = {"nll_dev": dv, "dnll_test": float(base_t - tv),
                     "adapter_bytes": int(50257 * r * 4)}
        print(f"  {name}: dev {dv:.4f} | test dNLL {base_t - tv:+.4f}",
              flush=True)
        if best_dev is None or dv < best_dev:
            best_dev, best_fw = dv, name
    out["fw_winner"] = best_fw
    p_fw = np.exp(lp_fw[best_fw])
    ...
    out["paired_combo_vs_fw"] = paired(p_fw, m_fw["p"], test)
    ...
    with open(f"results/fwcombo_{domain}.json", "w") as f:
