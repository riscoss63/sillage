...
\textbf{Voices.} The tool exposes two ways of answering from what it has read. The \emph{extractive} voice is \texttt{sillage ask}: each session is indexed as one document by the lexical passage index of \cite{sillage} (no embeddings, no model, no GPU), the question is the query. The \emph{generative} voices run Qwen3-0.6B over the memory state built by ingesting all $\sim$50 sessions: (a) the question alone --- the memory must both find and formulate; (b) the top-3 retrieved passages prepended, same state --- the RAG regime; (c) the same prompt on an empty state --- the control isolating the memory's contribution. Greedy decoding, 24 tokens.
